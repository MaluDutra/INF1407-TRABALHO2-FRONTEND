...
function criarCardMusica(musica) {
    ...
    if (musica.link) {
        const linkOuvir = document.createElement("a");
        linkOuvir.href = musica.link;
        linkOuvir.target = "_blank";
        linkOuvir.rel = "noopener noreferrer";
        linkOuvir.textContent = "Ouvir";
        card.appendChild(linkOuvir);
    }
    ...
    botaoExcluir.addEventListener("click", () => { void excluirMusica(musica); });
...
async function carregarMusicas() {
    ...
    try {
        const musicas = await apiRequest("/musicas/", "GET");
        limparContainer(lista);
        if (musicas.length === 0) {
